...
function makeTracker() {
    ...
    // Mock tracker.update to avoid trying to render DOM during tests
    tracker.update = () => {};
    return tracker;
}
...
describe('RevisionPage — sélection exacte des Juz', () => {
    ...
    it('sélectionne aussi une plage exacte pour un Hizb', () => {
        ...
        const hizb = HIZB_DATA[0];
        tracker.wizToggleRange(hizb.from, hizb.to, hizb.label, 'hizb');

        expect(tracker.state.wiz.ranges.find(r => r.label === HIZB_DATA[0].label)).toEqual({
