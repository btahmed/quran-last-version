...
function makeTracker() {
    ...
    tracker.state.wiz = { mode: 'build', expandedJuz: new Set(), expandedHizb: new Set(), selected: new Set(), selectedRanges: new Map(), ranges: [], importText: '', importError: null, copied: false };
    return tracker;
}
...
describe('RevisionPage — sélection exacte des Juz', () => {
    it('sélectionne uniquement la plage du Juz 1', () => {
        ...
        expect(tracker.state.wiz.ranges[0].from).toBe(1); expect(tracker.state.wiz.ranges[0].to).toBe(11);
    ...
    it('sélectionne aussi une plage exacte pour un Hizb', () => {
        ...
        expect(tracker.state.wiz.ranges).toContainEqual({ label: HIZB_DATA[0].label, from: HIZB_DATA[0].from, to: HIZB_DATA[0].to, type: 'hizb' });
