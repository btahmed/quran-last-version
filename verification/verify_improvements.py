...
def run():
    with sync_playwright() as p:
        ...
        page.route("**/api/token/", lambda r: r.fulfill(json={"access": "fake", "refresh": "fake"}))
        ...
        page.locator("#login-username").fill("teacher")
        page.locator("#login-password").fill("pass")

        # Click login and check for spinner immediately
        page.locator("#login-submit-btn").click()
        spinner = page.locator("#app-loading")
        if spinner.is_visible():
             print("✅ Spinner is visible")
        else:
             print("⚠️ Spinner missed or too fast")
